...
const OrdersDetails = ({setIsDetailsModal}) => {
    ...
    const fetchCompanyDetails = async () => {
        ...
        try {
            const res = await api.post('/api/order/orderCompany', {
            ...
            if (res.data.success) {
                setCompanyInvoices(res.data.data || []);
                console.log("Company invoices:", res.data.data);
            } else {
    ...
    const handlePrint = () => {
        ...
        WinPrint.document.write(` 
            <html>
                <head>
                    <title>Solitaire Hotel - Company Statement</title>
                    <style>
                        @page {
                            size: A4;
                            margin: 1.5cm;
    ...
    // Calculate totals
    const totalAmount = companyInvoices.reduce((acc, invoice) => acc + invoice.bills.totalWithTax, 0);
    const totalTax = companyInvoices.reduce((acc, invoice) => acc + invoice.bills.tax, 0);
    const totalPaid = companyInvoices.reduce((acc, invoice) => acc + invoice.bills.payed, 0);
            ...
            >
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        <div className="flex items-center gap-3">
                            ...
                            <div className="flex-1">
                                <h2 className='text-xl font-bold text-white'>Company Statement</h2>
                                <p className='text-emerald-100 text-sm'>Transaction history for {companyData.companyName}</p>
                            </div>
                        </div>
                ...
                <div ref={invoiceRef} className="hidden">
                    <div className="statement-container">
                        ...
                        <div className="statement-subtitle">Transaction History for {companyData.companyName}</div>
                        ...
                        <div className="guest-summary">
                            <div className="guest-grid">
                                <div className="guest-item">
                                    ...
                                    <div>
                                        <div className="guest-label">Company Name</div>
                                        <div className="guest-value">{companyData.companyName}</div>
                                    </div>
                                </div>
                                <div className="guest-item">
                                    <div className="guest-icon">💰</div>
                                    <div>
                                        ...
                                        <div className="guest-value">{companyData.balance.toFixed(2)} SD</div>
                                    </div>
                                </div>
                                <div className="guest-item">
                                    <div className="guest-icon">📋</div>
                                    <div>
                                        ...
                                        <div className="guest-value">{companyInvoices.length}</div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <table>
                            ...
                            <tfoot>
                                <tr>
                                    ...
                                    <td><strong>{companyInvoices.reduce((acc, inv) => acc + inv.bills.total, 0).toFixed(2)}</strong></td>
                                    <td><strong>{companyInvoices.reduce((acc, inv) => acc + inv.bills.tax, 0).toFixed(2)}</strong></td>
                        ...
                        <div className="summary-section">
                            ...
                            <div className="summary-card">
                                ...
                                <div className="value">{companyData.balance.toFixed(2)} SD</div>
                            </div>
                        </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div className="bg-white rounded-lg p-4 border border-emerald-200">
                            <div className="flex items-center gap-3 mb-2">
                                ...
                                <div>
                                    <p className="text-sm text-emerald-600 font-medium">Company</p>
                                    <h3 className="font-bold text-emerald-900 text-lg">{companyData.companyName}</h3>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="bg-white rounded-lg p-4 border border-emerald-200">
                            <div className="flex items-center gap-3 mb-2">
                                ...
                                <div>
                                    ...
                                    <h3 className={`font-bold text-lg ${companyData.balance === 0 ? 'text-emerald-600' : 'text-amber-600'}`}>
                                        {companyData.balance.toFixed(2)} SD
                                    </h3>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="bg-white rounded-lg p-4 border border-emerald-200">
                            <div className="flex items-center gap-3 mb-2">
                                ...
                                <div>
                                    ...
                                    <h3 className="font-bold text-emerald-900 text-lg">{companyInvoices.length}</h3>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div className="flex-1 overflow-auto p-5">
                    ...
                    {/* Empty State */}
                    {!loading && !error && companyInvoices.length === 0 && (
                        <div className="flex items-center justify-center py-12">
                            <div className="text-center">
                                ...
                                <p className="text-emerald-400 text-sm">This company has no transaction history</p>
                            </div>
                        </div>
                    )}
                                    ...
                                    <tfoot className="bg-gradient-to-r from-emerald-600 to-emerald-700 text-white">
                                        <tr>
                                            ...
                                            <td className="p-4 font-bold">
                                                {companyInvoices.reduce((acc, invoice) => acc + invoice.bills.total, 0).toFixed(2)}
                                            </td>
                                            <td className="p-4 font-bold">
                                                {companyInvoices.reduce((acc, invoice) => acc + invoice.bills.tax, 0).toFixed(2)}
                                            </td>
                                            <td className="p-4 font-bold">
                    ...
                    <div className="p-5 border-t border-emerald-100 bg-emerald-50/50">
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                            ...
                            <div className="bg-white rounded-lg p-4 border border-emerald-200">
                                ...
                                <p className={`text-xl font-bold ${companyData.balance === 0 ? 'text-emerald-600' : 'text-amber-600'}`}>
                                    {companyData.balance.toFixed(2)} SD
                                </p>
                            </div>
                        </div>
